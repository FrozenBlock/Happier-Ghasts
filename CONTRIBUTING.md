...
By contributing to Happier Ghasts, you agree with the [Developer Certificate of Origin (DCO)][DCO].
...
Use American English to keep consistency across Happier Ghasts.

If there is more than one acceptable spelling of the same word, first check if one word is already
being used in Happier Ghasts, FrozenLib, or in Mojang's Mappings, and if not, use the spelling that is most commonly used.
...
It allows to separate the actual name of the variable and the namespace, `happierGhasts`.

Fields marked as `@Unique` must be prefixed with `happierGhasts$` or `HAPPIER_GHASTS$` if the field is static and final.

Methods marked with `@Unique` must be prefixed with `happierGhasts$`.

Injector or modifier methods do not need to be prefixed. Fabric takes care of that.

## Licensing & DCO

Happier Ghasts is licensed under [GPL 3.0][LICENSE], and it has a [Developer Certificate of Origin (DCO)][DCO], which you are required to agree with to contribute.
...
[LICENSE]: ./LICENSE "Happier Ghasts license file"
